...
def main():
    ...
    if args.json_file:
        ...
        with open(args.json_file) as json_file_content:
            ...
            execute_command(export_variable_command, host_to_connect, username, password)
            full_host_name = f"{str(host_name)}.us.oracle.com"
            # my_overlay_hosts.append(host_name)
            print(full_host_name)
            for i in range(20):
                ...
                if not session:
                    ...
                    session = establish_ssh_session(full_host_name, username, root_password)
                if session:
                    exec_result = execute_command(grep_command, None, None, None, session)
                    print(f"Output for hostname : {full_host_name} result :{exec_result}")
                    if "Chef Client finished" in str(exec_result):
                        print(f"Overlay is completed for hostname : {full_host_name} and server is ready to connect")
                        print(f"Running patching script on {full_host_name}")
                        ...
                        if "OL_PATCH_SUCCESSFUL" in str(exec_result) or "SECURITY AND KERNEL PATCH IS ALREADY UPDATED" in str(exec_result):
                            print(f"Patching is complete on {full_host_name}")
                        else:
                            print(f"Patching did not finish or it has errors on {full_host_name}..")
                            exit(1)
                        print(f"=========proceeding with image cleaup on {full_host_name} ===========")
                        ...
                        print(f"=========image cleanup complete on {full_host_name}=====")
                        ...
                        print("Overlay not finished.trying again with root user for %s" % full_host_name)
                        time.sleep(300)
                print(f"unable to connect to hostname : {full_host_name} with username : {username}."
                      f"Trying to connect again...")
                time.sleep(300)
            print("Completed.")
            print("Image build finished for major version : %s and minor version: %s on the host"%(version, sub_version, full_host_name))
