...
log_file_name = "on_prem_image_build.log"
# Logging config
...
def main():
    try:
        ...
        install_command = "virt-install -d " \
                          ...
                          "netmask=255.255.248.0 gateway=10.240.224.1 dns=10.231.225.65' --virt-type kvm --disk " \
                          "path=/kvm/VirtualDisk/%s,size=50 " \
                          "--location http://pd-yum-slc-01.us.oracle.com/bootable/OracleLinux/%s/%s/%s/x86_64/ " \
        ...
        logger.info("Installation is in  . please wait....")
        time.sleep(1000)
        logger.info("----------------------------------------------------------\n"
                    f"{exec_result}\n"
                    "----------------------------------------------------------\n")
        ...
        if int(ping_response) == 0:
            ...
            if "Error" in str(exec_result):
                ...
                execute_command(_delete_host_command, session)
        ...
        for i in range(20):
            ...
            if session:
                ...
                if chef_completion_check in str(exec_result):
                    ...
                    copy_command = "cp -r /kvm/VirtualDisk/%s /kvm/VirtualDisk/%s_pipeline_22102021"% (host_name, build_type)
                    session = establish_ssh_session(host_to_connect, username, password)
