...
oit_logger = OITLoggerClient(
    api_url='https://logfuse-api.appoci.oraclecorp.com',
token='ZXlKMGVYQWlPaUpLVjFRaUxDSmhiR2NpT2lKSVV6STFOaUo5LmV5SmxlSEFpT2pFM01EWTFPVGt5TVRFc0ltbGhkQ0k2TVRZME16VXlOekl4TVN3aWMzVmlJam95ZlEuZXBMWXpSeFoxa3N3UnkwYUZsQ0Y5U3lrQ0RfSEtXaWN2aUMwOXpNeGl5MA=='
)
...
def get_native_image(image_ver,compartment_id):
    status,output = subprocess.getstatusoutput(f"sudo /root/bin/oci compute image list --config-file /root/.oci/config --compartment-id ocid1.compartment.oc1..aaaaaaaajvstndw4euzjmv6zgxz2hylmlrcgkyiqojhckmdenvepvqoacs3q --lifecycle-state AVAILABLE --all | grep -w \"display-name\" | grep -w \"Oracle-Linux-{image_ver}-[0-9]*\" | head -n 1")
    out1=output.split(':')[1]
    return out1.split("\"")[1]
    ...
    if args.json_file:
        ...
        with open(args.json_file) as json_file_content:
            ...
            for ver in test_images:
                ...
                # time.sleep(60)
                ver_val = f"peo-ol{ver.replace('.', 'u')}-qi"
                image_id = get_base_sp(ver_val.strip(), comp_ocid)
                if image_id is None:
                    ver_val = f"ee-ol{ver.replace('.', 'u')}-qi"
                    image_id = get_base_sp(ver_val.strip(), comp_ocid)
                    if image_id is None:
                        ver_val = get_native_image(ver,comp_ocid)
                        image_id = get_base_sp(ver_val.strip(), comp_ocid)
                time.sleep(60)
                if image_id:
                    val = {"image_name": ver_val, "image_id": image_id.strip(), "username": username,
